...
class StudentPoissonMixture:
    ...
    def pdf(
        ...
        for k in range(1, number_of_terms):
            total_mix_density = (
                ...
                + poisson.pmf(k, lamb)
                * 1
                / (kappa * sigma * np.sqrt(2 * np.pi * k))
                * quad(
                    self._student_integral, -10, 10, args=(x, k, mu, sigma, kappa, nu)
                )[0]
            )
    ...
    def cdf(
        ...
        return quad(
            ...
            -lower_limit,
            x,
            args=(mu, sigma, kappa, lamb, nu, number_of_terms),
        )[0]
    ...
    def ppf(
        ...
        lamb: float,
        nu: float,
        number_of_terms: int = 100,
        lower_limit: float = 50.0,
    ):
        target = prob
        sol = root_scalar(
            lambda x, *args: self.cdf(x, *args) - target,
